...
const virtual = createNamespace('virtual');
...
const LongListScroll: React.FC<ILongListScrollProps> = (props) => {
  const { activeIndex = 0, listData, rendNum, height = 500, countHeight, render } = props;
  const [listHeight, setListHeight] = useState(0);
  const [startOffset, setStartOffset] = useState(0);
  const [renderData, setRenderData] = useState<Array<Record<string, unknown>>>([]);
  ...
  return (
    ...
    >
      <div className={virtual.b('phantom')} style={{ height: listHeight + 'px' }}></div>
      <div className={virtual.b('list')} style={{ transform: `translate3d(0,${startOffset}px,0)` }}>
