...
describe('test menu and menuItem component', () => {
  // 将公用的变量放在这个函数，这个函数在每个case执行之前都会运行
  beforeEach(() => {
    wrapper = render(getMenuComponents(testProps));
    // 插入css文件
    wrapper.container.append(createStyleFile());
    // 获取更节点 data-testId = 'test-menu'
  ...
  it('should render correct Menu and MenuItem based on default props', () => {
    ...
    // 是否存在class
    expect(menuElement).toHaveClass('l-menu-container test');
    // 判断子元素的个数
  ...
  it('should render vertical mode when mode is set to vertical', () => {
    ...
    // // 判断当前menuElement有无vertical
    expect(menuElement).toHaveClass('l-menu-vertical');
  });
  it('should show dropdown items when hover on subMenu', async () => {
    // 判断当前元素出现在视野中
    expect(wrapper.queryByText('dropdown')).toBe(null);
    // 拿到drop
